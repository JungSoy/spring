...
public class CartController {
	...
	@GetMapping
	public String requetCartId(HttpServletRequest request) {
		String sessionId = request.getSession(true).getId();
		return "redirect:/cart/" + sessionId;
		
		// 경로/cart로 요청이 들어오면 세션id 값을 가져와서 cart/세션값으로 다시 호출
	}
	...
	public String requestCartList(@PathVariable(value="cartId") String cartId, Model model) {
		...
		return "cart";
		// 요청 uri가 /cart/장바구니아이디 로 get 으로 요청되면 처리 되는 메서드 해당 장바구니 아이디 cartId의
		// 모든 정보를 읽어서 cart 속성에 등록하고 뷰이름 cart 를 반환하므로 cart.jsp를 호출하게 된다.
	}
